...
- **Structured output**: every tool declares a BINDING `outputSchema` and returns `structuredContent` on every successful path, in the shared envelope `{query, summary, caveats, <payload>}` where the payload key names its contents (`layers`, `layer`, `buckets`, `rows`, `fields`, `values`, `candidates`). Caveats carry a stable `code` from `CAVEAT_CODES` in `plugins/arcgis/plugin.py` and are generated from ONE `_Caveats` list so the prose and the structured array cannot drift; tests validate real output against the advertised schema and assert every caveat message appears verbatim in the text. The spec's suggestion to also serialise the JSON into a text block is deliberately skipped (it would double token cost for an LLM consumer). Schemas are loose where the data is loose: `rows` are raw attributes, `total_matching` is null (not zero) when the count fails.
