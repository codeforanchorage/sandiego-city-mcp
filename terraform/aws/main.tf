...
resource "aws_lambda_function" "mcp_server" {
  ...
  # runtime (server/http_handler.py::_packaged_config_path). It is NOT passed
  # via OPENCONTEXT_CONFIG because the full config -- once the server
  # `instructions` block is included -- exceeds Lambda's hard 4KB limit on
  # total environment-variable size. Leaving the env var empty makes the
  # handler fall back to the packaged file.
  environment {
    variables = {
      OPENCONTEXT_CONFIG = ""
    }
